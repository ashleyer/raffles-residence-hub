...
export function ThankYouCarousel({
  ...
  return (
    <Carousel opts={{ align: "start", loop: notes.length > 2 }} aria-label={label} className="w-full">
        ...
        {notes.map((note) => (
          <CarouselItem key={note.id} className="pl-4 sm:basis-1/2 lg:basis-1/3">
            <figure className="flex h-full flex-col border border-border bg-card p-6">
              ...
              <blockquote className="mt-4 flex-1 text-pretty text-sm leading-relaxed">{note.body}</blockquote>
